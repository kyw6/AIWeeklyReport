...
public class TreeViewEditor {
    ...
    *  keep last location and
    ...
    public void unifyBackgrounds(Drawable background) {
        TreeViewContainer container = containerWeakReference.get();
        container.getTreeModel().doTraversalNodes(next -> {
            container.getTreeViewHolder(next).getView().setBackground(background);
        });
    }

    public void clearNodeMark() {
        TreeViewContainer container = containerWeakReference.get();
        container.getTreeModel().doTraversalNodes(NodeModel::clearMark);
    }
